...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            doOp(Pair(minIdx, acquire(minIdx, this)), Pair(maxIdx, acquire(maxIdx, this)), fromIndex < toIndex)
        }

        private fun doOp(minP: Pair<Int, AcquiredAccount?>, maxP: Pair<Int, AcquiredAccount?>, order: Boolean) {
            val fromAcc = if (order) minP.second else maxP.second
            val toAcc = if (order) maxP.second else minP.second
            if (fromAcc != null && toAcc != null) {
                if (verify(fromAcc, toAcc)) {
                    fromAcc.newAmount = fromAcc.amount - amount
                    toAcc.newAmount = toAcc.amount + amount
                }
            }
            completed = true
            release(minP.first, this)
            release(maxP.first, this)
        }

        private fun verify(fromAcc: AcquiredAccount, toAcc: AcquiredAccount): Boolean =
            if (fromAcc.amount < amount) {
                errorMessage = "Underflow"
                false
            } else if (toAcc.amount + amount > MAX_AMOUNT) {
